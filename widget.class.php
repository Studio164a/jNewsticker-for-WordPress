...
{    
    function jNewsticker_Widget() {
        ...
        $this->WP_Widget('jNewsticker_Widget', __('Newsticker', 'jnews'), $widget_ops, $control_ops);
    ...
    function form($instance) {                      
        ...
        <p>
            <?php if ( count ( $newstickers ) ) : ?>
                ...
                <label for="<?php echo $this->get_field_id('newsticker_id'); ?>"><?php _e('Select news ticker:', 'jnews') ?></label>
            ...
            <?php else : ?>
                <?php printf( __('No news tickers have been configured yet. %sCreate one now%s', 'jnews'), '<a href="/options-general.php?page=jnewsticker">', '</a>') ?>
            <?php endif ?>
        </p>                
        <?php
    }
